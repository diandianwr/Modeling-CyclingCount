...
To better support our choice on \texttt{TempMax} and \texttt{PrecipProb}, the correlation scatter matrix is depicted as in Figure~\ref{fig:temp_corr}. It is shown that all temperature related variables have similar correlations with bike counts, and very high correlation with each other (more than 0.9). To avoid collinearity it is reasonable to keep only \texttt{TempMax} because it has the highest correlation with bike counts. Similarly, in the scatter matrix Figure~\ref{fig:precip_corr}, it is noted that correlation between the probability of precipitation and bike count is -0.452, strongest of all precipitation-related variables. At the same time, there exist high correlations between \texttt{PrecipProb} and other variables such as \texttt{precipIntensityMax}, \texttt{precipIntensity}, and \texttt{humidity}. To avoid collinearity we decided to keep \texttt{PrecipProb} in our model.
...
Day of the week \texttt{dow} was added due to its presence in the literature, as well as an apparent weekly pattern revealed visually by zooming into the time series plot (Figure~\ref{fig:descriptionbc}). These data were coded as a set of Boolean dummy variables, excluding Friday as the reference category. In addition, the weekend or not flag \texttt{Weekend} is included since we are interested in understanding the traveling behaviour of bicyclists at Fremont Bridge, mostly commuters. It is expected to have big variation in bike counts between weekends and weekdays but not so much between individual day of the week. 
...
\texttt{dow} & Day of the week dummy variable (from Saturday to Thursday) \\
...
Box plots for categorical seasonal factors against daily bike count are depicted in Figure~\ref{fig:cat_bxplot}. Visually, we are able to observe the difference in their means, suggesting these variables have explanatory power to account for variation in bike count. Formal statistical tests are conducted to confirm their significance in the latter Section~\ref{sec:intrepofresult}.

\subsection{Summary of this section}

Combining above analysis, a set of explanatory variables is extracted from the original dataset. Their names and descriptions are summarized in Table~\ref{tb:variables}. Note that this list of variables are \emph{not} the final list of variables used in the model. It represents the group of candidate variables that will be further investigated in the latter sections. 


\section{Modeling Approach}
\label{sec:modelApproach}

There are multiple approaches to model the relationship between weather and seasonal variables and daily cyclist volumes. We begin with the regression models, which consists of the linear model and the count model (Poisson model). The choice of preferred model is then made based on detailed analysis of residual error and goodness-of-fit. To account for the nonlinear relationship between weather and bike counts, the generalized additive model (GAM) is adopted and briefly reviewed in this section. Lastly, a autoregressive integrated moving average (ARIMA)-type model is proposed for the prediction of daily bike counts, where the autocorrelation is taken into account. 

\subsection{Regression Model}

\subsubsection{Liner Model}
...
Note that besides the simplicity of Poisson model, another advantage with Poisson model is its ease of interpretation. With one unit increase in one of the explanatory variable, a percentage change in the dependent variable can be straightforwardly calculated. Therefore, the Poisson model is also considered in this thesis. 
