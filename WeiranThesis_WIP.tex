...
\setcounter{page}{-1}
\abstract{%
Cycling has been proven to be beneficial to cyclists' heath as well as a sustainable transportation system of urban cities. Therefore policy makers and urban planners all over the world have been promoting bicycling. City of Seattle in the northwest region has been implementing new policies and programs to create a bike friendly environment and aims to quadruple
ridership by 2030. Empirically confirming such growth in Seattle will help to justify current and future investment in bicycle infrastructure and programs in Seattle.  This study uses Seattle metropolitan area cyclist count data to quantify cycling trend and examine their relationship between seasonal and weather factors. First, a systematic approach is taken to identify the explanatory variables and their appropriate forms of transformation. Then different choices of model specifications are investigated and compared to best capture the relationship between bike counts and factors. Specifically, non-linearity, discontinuity and interaction items are taken into account. Results are interpreted with intuitive visualization using counterfactual simulations. Furthermore, a predictive model is proposed to estimate daily count in the future. Autoregressive Integrated Moving Average (ARIMA) model is used to account for autocorrelation. Its predictive performance is evaluated using cross validation. Finally, proposed methodology is applied to multiple locations in Seattle and identify their unique bicycle travel patterns. This research will help policy makers and transportation planners to better understand the factors that could drive the bike demand and influence bike travel behavior.}
...
\section{Background}
Cycling is a fun and healthy choice of transportation in urban city environments which can be used for all ages and abilities. It is becoming an increasingly important mode in a comprehensive sustainable transportation system.  A country-wide increase in bicycling in urban cities has been seen recently. %(nationall data?????)% 
...
A summary of important literature surveyed above with their selection of variables and methods are presented in the following Table. %These literature together suggest an opportunity for further model development base around long-term automated counts utilizing appropriate statistical methods. How seasonal factors influence bicycle flow needs to be examined in data that last more than a year. One limitation present in much of the past literature is that few discuss goodness of fit of their modeling. Further a model that can better describe and forecast the bicycle count in longitudinal form is necessary to be developed. Models for count data with better estimation methods offer some promise. 

\begin{figure*}
\vspace{-15pt}
  \includegraphics[width=1\textwidth]{figures/literature}
   \label{tb:lit}
%   \caption{Summary of existing literature: variable and method}
\end{figure*}
...
% After the data are assembled through various source as described in Section~\ref{sec:data}, it consists of 34 variables. 26 of them are related to weather, 7 are related with season. Figure~\ref{fig:descriptionbc} provides a visual summary of the bike counts data. Some apparent outliers are visible at the rightmost portion of the histogram (more than 6000). The two highest counts occurred the Monday and Tuesday preceding the beginning of National Bike to Work Month. And the third highest count occurred on National Bike to Work Day.
...
% , which uses the built-in nonparametric smooth splines, and a backfitting algorithm to combine different fitting methods. 
...
%The fitted GAM model is summarized in Table~\ref{gamtable}. \textbf{Add one paragraph on the GAM model summary}. As seen in Figure~\ref{fig:smooth_resid}, there is no discernible pattern in the Residual vs. Fitted value plot. Furthermore, the Normal Q-Q plot suggest that the residual follows an approximately Gaussian distribution. 
...
%When the maximum daily temperature is below $40^{\circ}$F, the bike count will approximately remain the same; 
...
\\[-4ex] & \multicolumn{1}{c}{Model0} & \multicolumn{1}{c}{Model1} & \multicolumn{1}{c}{Model2}\\ 
\hline \\[-1.8ex] 
...
\cline{2-4} 
\\[-4ex] & \multicolumn{1}{c}{Model0} & \multicolumn{1}{c}{Model3} & \multicolumn{1}{c}{Model4}\\ 
\hline \\[-1.8ex] 
...
\hline \\[-4ex] 
 & \multicolumn{3}{c}{\textit{Dependent variable:$\sqrt{\text{Bike Count}}$}} \\ 
\cline{2-4} 
\\[-4ex] & \multicolumn{1}{c}{Model0} & \multicolumn{1}{c}{Model5} & \multicolumn{1}{c}{Model6}\\ 
\hline \\[-1.8ex] 
 TempMaxSq & -0.006^{***}$ $(0.001) & -0.006^{***}$ $(0.001) & -0.005^{***}$ $(0.001) \\ 
...
\\[-5ex]\hline 
\hline \\[-4ex] 
 & \multicolumn{2}{c}{\textit{Dependent variable:$\sqrt{\text{Bike Count}}$}} \\ 
\cline{2-3} 
\\[-4ex] & \multicolumn{1}{c}{Model0} & \multicolumn{1}{c}{Model7}\\ 
\hline \\[-4ex] 
...
R$^{2}$ & \multicolumn{1}{c}{0.866} & \multicolumn{1}{c}{0.887} \\ 
Adjusted R$^{2}$ & \multicolumn{1}{c}{0.866} & \multicolumn{1}{c}{0.886} \\ 
AIC & \multicolumn{1}{c}{6930} & \multicolumn{1}{c}{6755} \\ 
BIC & \multicolumn{1}{c}{6976} & \multicolumn{1}{c}{6861} \\ 
\hline 
\hline \\[-4ex] 
...
\cline{2-3} 
\\[-3ex] & \multicolumn{1}{c}{Square root transformation model} & \multicolumn{1}{c}{ARIMA(1,0,1)} \\ 
\hline \\[-1.8ex] 
 ...
 \hline \\[-1.8ex] 
Observations & \multicolumn{1}{c}{1,157} & \multicolumn{1}{c}{1,157} \\ 
R$^{2}$ & \multicolumn{1}{c}{0.866} & \multicolumn{1}{c}{0.890} \\ 
Adjusted R$^{2}$ & \multicolumn{1}{c}{0.866} & \multicolumn{1}{c}{0.889} \\ 
Akaike Inf. Crit. & \multicolumn{1}{c}{6986} & \multicolumn{1}{c}{6707} \\ 
Bayesian Inf. Crit. & \multicolumn{1}{c}{7045} & \multicolumn{1}{c}{6729}  \\ 
\hline 
\hline \\[-1.8ex] 
\textit{Note:}  & \multicolumn{2}{r}{$^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01} \\ 
\end{tabular} 
...
%Figure~\ref{fig:arimamonth} shows the monthly aggregated bike counts from January 2013 to December 2015. Note that the first 24 points correspond to the actual bike count, and the red lines are the predicted bike volumes with the 95\% prediction interval in shades.
...
% \subsection{Validation of the predictive model}
% \textbf{WIP-AVP, cross validation, both in-sample and out-of-sample, mse, etc).}
% In this section, we are interested to evaluate the predictive power of the two models under consideration: Model0~\eqref{}, and the ARIMA model~\eqref{}.
...
% For comparison of the predictive power, we also include the arima model for the lm model as well. See Figure~\ref{fig:avp_arima1} for cross validation and Figure~\ref{fig:avp_arima2} for fitted values.
...
%  \caption{Effect of temperature on counts for five locations, with shaded 95\% confidence interval. }
%  \label{fig:Sim_temp}
% \end{figure*}
...
%   \caption{Effect of precipitation on counts for five locations, with shaded 95\% confidence interval. }
% \end{figure*}
